...
//wait while camera is recording
  ...
  short buffer_nums[BUFFER_ARRAY_SIZE]{-1, -1, -1, -1};
  WORD *buffer_addrs[BUFFER_ARRAY_SIZE]{nullptr, nullptr, nullptr, nullptr};

  for (int b = 0; b < BUFFER_ARRAY_SIZE; ++b)
  {
    pco_error = PCO_AllocateBuffer(pco_cam, &buffer_nums[b], buffer_size, &buffer_addrs[b], nullptr); // no event parameter
    ERRABORT(pco_cam, pco_error);
  }
  ...
  for(int b=0;b<BUFFER_ARRAY_SIZE;b++)
  {
    if(ValidImageCnt >= img_set)
    {
      pco_error = PCO_AddBufferEx(pco_cam,img_set,img_set, buffer_nums[b], width, height, 16);
      ERRABORT(pco_cam, pco_error);
      img_set++;
    }
  }

  DWORD current_status_sdk, current_status_driver;
  short current_buffer_num = -1;

  //this example does readout only first IMAGE_SEQUENCE_COUNT images, adapt this e.g. use ValidImageCnt
  for(DWORD img_num = 1; img_num <= IMAGE_SEQUENCE_COUNT; img_num++)
  {
    ...
    pco_error = PCO_WaitforNextBufferNum(pco_cam, &current_buffer_num, IMAGE_TIMEOUT_MS);
    ...
    printf(" pco buffer %d ", current_buffer_num);

    current_status_sdk = PCO_ERROR_WRONGVALUE;
    current_status_driver = PCO_ERROR_WRONGVALUE;
    pco_error = PCO_GetBufferStatus(pco_cam, current_buffer_num, &current_status_sdk, &current_status_driver);
    ...
    for (int b = 0; b < BUFFER_ARRAY_SIZE; ++b)
    {
      if(buffer_nums[b]==current_buffer_num)
      {
        bufadr=buffer_addrs[b];
        break;
      }
    }
    if(bufadr)
    {
      char file_path[256];
      snprintf(file_path, 256, "image_buf_mono16_(%02d).tif", img_num);
      write_tiff(file_path,bufadr, width, height, 16);
    }
    #endif

    if(ValidImageCnt >= img_set)
    {
      pco_error = PCO_AddBufferEx(pco_cam,img_set,img_set, current_buffer_num, width, height, 16);
      img_set++;
    }
  }
  ...
  for (int b = 0; b < BUFFER_ARRAY_SIZE; ++b)
  {
    pco_error = PCO_FreeBuffer(pco_cam, buffer_nums[b]);
    ERRABORT(pco_cam, pco_error);
  }
