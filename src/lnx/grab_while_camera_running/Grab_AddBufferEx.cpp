...
#define BUFFER_ARRAY_SIZE 4
  ...
  for (int b = 0; b < BUFFER_ARRAY_SIZE; ++b)
  {
    pco_error = PCO_AddBufferEx(pco_cam, 0, 0, buffer_nums[b], width, height, 16);
    ERRABORT(pco_cam, pco_error);
  }
  ...
  WORD *current_buffer_addr = nullptr;
  HANDLE not_used;
...
#define IMAGE_SEQUENCE_COUNT 10
  for (int img_num = 0; img_num < IMAGE_SEQUENCE_COUNT; ++img_num)
  {
    ...
    else
    {
      ...
      pco_error = PCO_AddBufferEx(pco_cam, 0, 0, current_buffer_num, width, height, 16);
      ERRBREAK(pco_cam, pco_error);

      break;
    }

    pco_error = PCO_GetBuffer(pco_cam, current_buffer_num, &current_buffer_addr, &not_used);
    ERRBREAK(pco_cam, pco_error);

#ifdef FILEFUNCTION
    char file_path[256];
    snprintf(file_path, 256, "image_buf_mono16_(%02d).tif", img_num);
    write_tiff(file_path, current_buffer_addr, width, height, 16);
#endif
    pco_error = PCO_AddBufferEx(pco_cam, 0, 0, current_buffer_num, width, height, 16);
    ERRBREAK(pco_cam, pco_error);
  }
